...
for(j in 1:rep){
  ...
  for(i in 1:10){
    ...
    if(i == 1){
      ...
      index <- sample(1:5,2)
      endo <- rep(1,5)
      endo[index] <- runif(2,min = .8,max = 1.6)
      
      r1 <- rnorm(n,mean = 0,sd = endo[1])
      r2 <- rnorm(n,mean = 0,sd = endo[2])
      r3 <- rnorm(n,mean = 0,sd = endo[3])
      r4 <- rnorm(n,mean = 0,sd = endo[4])
      r5 <- rnorm(n,mean = 0,sd = endo[5])
